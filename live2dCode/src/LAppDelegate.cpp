...
// INFO 新增移动窗口逻辑
HWND LAppDelegate::_hw = 0;

bool LAppDelegate::_canMove = false;

POINT LAppDelegate::_deviationPoint;

LONG LAppDelegate::_windowWidth = GetSystemMetrics(SM_CXSCREEN);

LONG LAppDelegate::_windowHeight = GetSystemMetrics(SM_CYSCREEN);

// INFO
void LAppDelegate::PosHandle() 
{
    if (_canMove) {
        POINT pt;
        ::GetCursorPos(&pt);
...
bool LAppDelegate::Initialize(int x,int y)
{
    if (DebugLogEnable)
    {
    ...
    // INFO 窗口不带边框且置顶
        ...
        //_windowHandle = CreateWindowEx(WS_EX_TOOLWINDOW | WS_EX_LAYERED | WS_EX_TOPMOST, ClassName, ClassName,
        //    WS_POPUP | WS_VISIBLE | WS_CLIPSIBLINGS | WS_OVERLAPPED,
        //    GetSystemMetrics(SM_CXSCREEN) - RenderTargetWidth, GetSystemMetrics(SM_CYSCREEN) - RenderTargetHeight,
        //    RenderTargetWidth, RenderTargetHeight, NULL, NULL, NULL, NULL);

        //SetLayeredWindowAttributes(_windowHandle, RGB(0, 0, 0), 100, LWA_COLORKEY);
    int _windowWidth = RenderTargetWidth;
    int _windowHeight = RenderTargetHeight;

    int windowX = x - _windowWidth / 2;  // 计算窗口左上角的 x 坐标
    int windowY = y - _windowHeight / 2;  // 计算窗口左上角的 y 坐标

    _windowHandle = CreateWindowEx(WS_EX_TOOLWINDOW | WS_EX_LAYERED | WS_EX_TOPMOST , ClassName, ClassName,
        WS_POPUP | WS_VISIBLE | WS_CLIPSIBLINGS | WS_OVERLAPPED,
        windowX, windowY, _windowWidth, _windowHeight, NULL, NULL, NULL, NULL);
    SetWindowText(_windowHandle, "VirtualPet");
    SetLayeredWindowAttributes(_windowHandle, RGB(0, 0, 0), 100, LWA_COLORKEY);
...
void LAppDelegate::Run()
{
    ...
    } while (msg.message != WM_QUIT && !isFinished);
...
LRESULT WINAPI LAppDelegate::MsgProc(HWND wnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CONTEXTMENU:
    {
       LAppLive2DManager::GetInstance()->isRightTouch = true;
       break;
    }
    case WM_COMMAND:
        break;
    ...
    case WM_LBUTTONDOWN:
        if (::s_instance != NULL)
        {
...

            {
                ...
                LAppLive2DManager::GetInstance()->isLeftTouch = true;
                ...
                if (_canMove == false) {
                    ...
                    POINT devPos;
                    devPos.x = pos.x - rect.left;
                    devPos.y = pos.y - rect.top;
                    _deviationPoint = devPos;
                }
